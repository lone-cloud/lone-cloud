I build tools I actually use.

---

**[gerbil](https://github.com/lone-cloud/gerbil)** &mdash; desktop app for running LLMs on your own hardware. no cloud, no telemetry, no bullshit.

**[polly](https://github.com/lone-cloud/polly)** &mdash; minimalist Signal fork. strips stories and bloat, hides FCM in favor of UnifiedPush. arm64-only, Android 14+.

**[prism](https://github.com/lone-cloud/prism)** + **[prism-android](https://github.com/lone-cloud/prism-android)** &mdash; self-hosted UnifiedPush notification gateway and Android client.

**[snowflake-dashboard](https://github.com/lone-cloud/snowflake-dashboard)** &mdash; metrics dashboard for the Tor snowflake proxy.

---

everything runs locally or on my own server. nothing on google.
